...
class Boids(object):
    ...
    def __init__(self):
        ...
        # Construct boids as a array of numpy arrays
        boids_x = np.array([random.uniform(INIT_CONFIG["x_min"], INIT_CONFIG["x_max"]) for x in range(self.num_boids)])
        boids_y = np.array([random.uniform(INIT_CONFIG["y_min"], INIT_CONFIG["y_max"]) for x in range(self.num_boids)])
        boid_x_velocities = np.array([random.uniform(INIT_CONFIG["vx_min"], INIT_CONFIG["vx_max"]) for x in range(self.num_boids)])
        boid_y_velocities = np.array([random.uniform(INIT_CONFIG["vy_min"], INIT_CONFIG["vy_max"]) for x in range(self.num_boids)])
